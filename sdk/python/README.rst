...
::
   ...
   git clone https://github.com/pulumi/pulumi-tf-provider-boilerplate pulumi-fastly

Second, replace references to ``fastly`` with the name of your provider:
...
-  ``GO111MODULE=on go get github.com/terraform-providers/terraform-provider-fastly``
   (where ``fastly`` is the name of the provider)
...
The following configuration points are available for the ``fastly``
provider:

-  ``fastly:apiKey`` (environment: ``XYZ_API_KEY``) - the API key for
   ``fastly``
-  ``fastly:region`` (environment: ``XYZ_REGION``) - the region in which
   to deploy resources
