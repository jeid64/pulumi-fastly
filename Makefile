PROJECT_NAME := fastly Package
include build/common.mk

PACK             := fastly
PACKDIR          := sdk
PROJECT          := github.com/jeid64/pulumi-${PACK}
...
prepare::
	...
	if [[ "${OS}" != "Darwin" ]]; then \
		sed -i 's,github.com/pulumi/pulumi-fastly,${REPOSITORY},g' go.mod; \
...
tfgen::
	go install -ldflags "-X github.com/jeid64/pulumi-${PACK}/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${TFGEN}

provider::
	go install -ldflags "-X github.com/jeid64/pulumi-${PACK}/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${PROVIDER}
